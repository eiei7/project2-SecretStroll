...
def test_sign(benchmark):
    ...
    attributes = [G1.order().random() for _ in range(list_len)]
    Sk, Pk = generate_key(attributes)
    msgs = Pk.attributes_to_bytes(dict(enumerate(attributes)))
    sigma = sign(Sk, msgs)
...
def test_sign_fail():
    ...
    Sk, Pk = generate_key(attributes)
    msgs = Pk.attributes_to_bytes(dict(enumerate(attributes)))
    fake_msgs = Pk.attributes_to_bytes(dict(enumerate(fake_attributes)))
    sigma = sign(Sk, msgs)
...
def test_issue_request(benchmark):
    ...
    # start request
    issueRequest = create_issue_request(Pk, user_attributes)
...
def test_sign_issue_request(benchmark):
    ...
    # start request
    issueRequest = create_issue_request(Pk, user_attributes)
    blind_sig = sign_issue_request(Sk, Pk, issueRequest, issuer_attributes)
    # benchmark(sign_issue_request, Sk, Pk, issueRequest, issuer_attributes)
    # if no error thrown, success
...
def test_verify_issue_request(benchmark):
    ...
    user_attributes, issuer_attributes = randomly_split_attributes(attributes)
    issueRequest = create_issue_request(Pk, user_attributes)
    assert Proof(pk=Pk).issuer_verify(issueRequest)
    # benchmark(Proof(pk=Pk).issuer_verify(issueRequest), issue_req, Pk)
...
def test_obtain_credential():
    ...
    issueRequest = create_issue_request(Pk, user_attributes)
    blind_sig = sign_issue_request(Sk, Pk, issueRequest, issuer_attributes)
    anon_cred = obtain_credential(Pk, blind_sig)
    # if no error thrown, success
...
def test_obtaining_credential_fail():
    ...
    with pytest.raises(ValueError):
        fake_keys = generate_key(attributes)
        obtain_credential(fake_keys[1], blind_sig)
    with pytest.raises(ValueError):
        fake_keys = generate_key(attributes)
        fake_state, fake_issue_request = create_issue_request(
            fake_keys[1], user_attributes
